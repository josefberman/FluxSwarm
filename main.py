...
def main():
    for iter_max_force in range(10, 101, 10):
        ...
        swarm = Swarm(num_x=3,
                      ...
                      member_max_force=iter_max_force
                      )  # density in mg/mm^3, force in mg*mm/s^2
        # max force 0.017 mg*mm/s^2
        inflow = Inflow(frequency=2 * np.pi,
                        amplitude=200,
                        radius=sim.length_y / 2,
                        center_y=sim.length_y / 2)
        inflow.center_x = 0
        ...
        velocity_field = StaggeredGrid(0,
                                       boundary=boundary,
                                       bounds=box,
                                       x=sim.resolution[0],
                                       y=sim.resolution[1])
        ...
        log_parameters(folder_name=folder_name,
                       sim=sim,
                       swarm=swarm,
                       inflow=inflow,
                       fluid=fluid)
