...
def plot_save_current_step(time_step: int, folder_name: str, v_field: Field, p_field: Field, inflow_field: Field,
    ...
    ax_handlers.append(plot_scalar_field(field=fields[0], ax=axes[0], length_x=sim.length_x, length_y=sim.length_y,
                                         title=field_names[0]))
    fig.colorbar(ax_handlers[-1], ax=axes[0], orientation='vertical', pad=0.04, fraction=0.02)
    # Velocity - y component
    ax_handlers.append(plot_scalar_field(field=fields[1], ax=axes[1], length_x=sim.length_x, length_y=sim.length_y,
                                         title=field_names[1]))
    fig.colorbar(ax_handlers[-1], ax=axes[1], orientation='vertical', pad=0.04, fraction=0.02)
    # Pressure
    ax_handlers.append(plot_scalar_field(field=fields[2], ax=axes[2], length_x=sim.length_x, length_y=sim.length_y,
                                         title=field_names[2]))
    fig.colorbar(ax_handlers[-1], ax=axes[2], orientation='vertical', pad=0.04, fraction=0.02)
    ax_handlers.append(plot_scalar_field(field=fields[3], ax=axes[3], length_x=sim.length_x, length_y=sim.length_y,
                                         title=field_names[3]))
...
def create_animation_frame_row(fig: plt.Figure, axis, sim: Simulation, imshow_data: np.ndarray,
    ...
    plot_handler, = axis[1].plot(np.linspace(0, sim.length_x, sim.resolution[0] + 1), plot_data, c='k')
...
def animate_save_simulation(sim: Simulation, folder_name: str) -> None:
    ...
    im1, plot1 = create_animation_frame_row(fig=fig, axis=ax[0], sim=sim,
                                            ...
                                            plot_data=velocity_data[0]['data'][:, int(sim.resolution[1] / 2), 0],
    ...
    im2, plot2 = create_animation_frame_row(fig=fig, axis=ax[1], sim=sim,
                                            ...
                                            plot_data=velocity_data[0]['data'][:, int(sim.resolution[1] / 2), 1],
    ...
    im3, plot3 = create_animation_frame_row(fig=fig, axis=ax[2], sim=sim,
                                            ...
                                            plot_data=pressure_data[0]['data'][:, int(sim.resolution[1] / 2)],
    ...
    im4, plot4 = create_animation_frame_row(fig=fig, axis=ax[3], sim=sim,
                                            ...
                                            plot_data=inflow_data[0]['data'][:, int(sim.resolution[1] / 2)],
    ...
    def update(frame):
        ...
        plot1.set_ydata(velocity_data[frame]['data'][:, int(sim.resolution[1] / 2), 0])
        plot2.set_ydata(velocity_data[frame]['data'][:, int(sim.resolution[1] / 2), 1])
